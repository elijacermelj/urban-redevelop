...
export function TopLocationsList({
  ...
  return (
    <div
      className={cn(
        "flex flex-col border-border bg-surface/80 backdrop-blur-md",
        mobileFullscreen
          ? "fixed inset-0 z-50 h-full w-full bg-surface"
          : "h-full w-[380px] shrink-0 border-l",
      )}
    >
      <div
        className={cn(
          "flex items-center justify-between border-b border-border px-5 py-4",
          mobileFullscreen && "bg-surface/95",
        )}
      >
        <div>
          ...
          <div className="font-display text-base font-semibold">{locations.length} priporočil</div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={onOpenCompare}
            disabled={comparedIds.length < minCompareCount}
            className={cn(
              "inline-flex cursor-pointer items-center gap-1.5 rounded-md border px-3 py-1.5 text-xs font-medium transition-all disabled:cursor-not-allowed",
              comparedIds.length >= minCompareCount
          ...
          >
            ...
            Primerjaj ({comparedIds.length}/{maxCompareCount})
          </button>
          {mobileFullscreen && (
            <button
              type="button"
              ...
              aria-label="Zapri priporočila"
              title="Zapri priporočila"
            >
              <X className="size-4" />
            </button>
          )}
      ...
      <ol
        className={cn(
          ...
          mobileFullscreen && "pb-[calc(0.75rem+env(safe-area-inset-bottom))]",
        )}
      >
        {locations.length === 0 && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="truncate text-sm font-semibold leading-tight">{loc.name}</h3>
                  </div>
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={cn(
                        "cursor-pointer rounded border px-2 py-0.5 text-[10px] font-medium transition-colors disabled:cursor-not-allowed",
                        compared
