...
import { X, MapPin, Train, Wrench, Droplets, Building, Recycle, GitCompare } from "lucide-react";
...
export function LocationDetailDrawer({
  ...
  const compared = comparedIds.includes(location.id);
  const canAddCompare = compared || comparedIds.length < maxCompareCount;

  return (
    <>
      <div
        className="fixed inset-0 z-40 bg-background/45 animate-in fade-in lg:pointer-events-none lg:bg-transparent"
        onClick={onClose}
      />
      <aside className="fixed right-0 top-0 z-50 h-full w-full max-w-[480px] overflow-y-auto border-l border-border bg-surface shadow-[var(--shadow-elevated)] animate-in slide-in-from-right">
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-border bg-surface/95 px-6 py-4">
          <div className="min-w-0 flex-1">
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
              {location.district} · {location.municipality}
            </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                "inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-md border px-2.5 text-[11px] font-medium transition-colors disabled:cursor-not-allowed",
                compared
                  ? "border-accent bg-accent/20 text-accent"
                  : "border-border bg-surface-elevated text-muted-foreground hover:border-accent/50 hover:text-accent disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:border-border disabled:hover:text-muted-foreground",
              )}
              title={
                compared
                  ? "Odstrani iz primerjave"
                  : canAddCompare
                    ? "Dodaj v primerjavo"
                    : `Največ ${maxCompareCount} lokacije v primerjavi`
              }
            >
              <GitCompare className="size-3.5" />
              {compared ? "V primerjavi" : canAddCompare ? "+ Primerjaj" : "Primerjava polna"}
            </button>
            <button
              onClick={onClose}
        ...
        <div className="space-y-6 px-6 py-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Metric
                icon={MapPin}
                label="V naselju"
                value={m.within_settlement ? "Da" : m.edge_of_settlement ? "Rob" : "Ne"}
              />
          ...
          <div>
            ...
            <div className="space-y-2">
              {(Object.keys(CRITERIA_LABELS) as Array<keyof typeof CRITERIA_LABELS>).map((k) => {
                  ...
                  <div key={k} className="space-y-1">
                    ...
                    <div className="h-1.5 overflow-hidden rounded-full bg-surface">
                      <div
                        ...
                        style={{
                          width: `${v * 100}%`,
                          backgroundColor: isPenalty ? "var(--destructive)" : scoreColor(v),
                        }}
                      />
                    </div>
                  </div>
